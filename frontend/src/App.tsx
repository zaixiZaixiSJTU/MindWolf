...
import { Moon, Sun, Skull, Wifi, WifiOff } from 'lucide-react';
...
export default function App() {
  ...
  const [speeches, setSpeeches] = useState<
    { pid: number; text: string; ts: number }[]
  >([]);
  const [thinkingChunks, setThinkingChunks] = useState<Map<number, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
      ...
      on('NIGHT_RESULT', (msg: WSMessage) => {
        ...
        setLogs(prev => [
          ...prev.slice(-199),
          {
            ...
            deaths:
              deathInfo ||
              (deaths?.length ? `${deaths.length} dead` : 'Peaceful night'),
          },
        ]);
      }),
    );
    ...
    unsubs.push(
      on('PLAYER_SPEECH', (msg: WSMessage) => {
        const p = msg.payload as Record<string, unknown>;
        setSpeeches(prev => [
          ...prev.slice(-5),
          {
    ...
    unsubs.push(
      on('THINKING_CHUNK', (msg: WSMessage) => {
        const p = msg.payload as Record<string, unknown>;
  ...
  const startGame = useCallback(
    async (
      ...
      try {
        ...
        const startResp = await fetch('http://localhost:8765/game/start', {
          method: 'POST',
        });
        ...
        const mockPlayers: PlayerInfo[] = Array.from(
          { length: 12 },
          (_, i) => ({
            id: i + 1,
            name: `Player ${i + 1}`,
    ...
    >
      ...
      <header
        className="flex items-center justify-between px-6 py-3
                   border-b border-slate-700/60 bg-slate-900/50
                   backdrop-blur-sm"
      >
        <div className="flex items-center gap-3">
          ...
          {gameState.players.length > 0 && (
            <span className="text-[11px] text-slate-600 bg-slate-800
                             px-2 py-0.5 rounded-full">
              {gameState.players.filter(p => p.is_alive).length} alive
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 text-sm text-slate-400">
          <div className="flex items-center gap-2">
            <span className="text-slate-500 text-xs">R{gameState.round}</span>
            <span
          ...
          >
            <motion.div
              ...
              className="bg-slate-800 border border-slate-700 rounded-2xl
                         p-8 text-center max-w-md w-full mx-4 shadow-2xl"
            >
              <motion.div
                ...
                {gameState.players.map(p => (
                  <div
                    ...
                    className="flex justify-between items-center
                               bg-slate-900/50 rounded-lg px-3 py-1.5"
                  >
                    <span className="flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full ${
                          p.is_alive ? 'bg-green-400' : 'bg-red-400'
                        }`}
                      />
                      {p.name || `Player ${p.id}`}
                    </span>
